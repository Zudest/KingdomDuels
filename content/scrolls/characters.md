+++
title = 'Characters'
toc = false
+++

<div style="display: flex; justify-content: center; gap: 5px;">
{{< imagetext src="/img/storyimg/M1.png" link="/yugioh-scrolls/" alt="Chapters" caption="Chapters" width="140" >}}
{{< imagetext src="/img/storyimg/M2.png" link="/scrolls/world/" alt="Setting" caption="Setting" width="140" >}}
{{< imagetext src="/img/storyimg/M3.png" link="/scrolls/timeline/" alt="Timeline" caption="Timeline" width="140" >}}
{{< imagetext src="/img/storyimg/M4.png" link="/scrolls/duels/" alt="Duels" caption="Duels" width="140" >}}
{{< imagetext src="/img/storyimg/M5.png" link="/scrolls/characters/" alt="Characters" caption="Characters" width="140" >}}
</div>

>_**[Yugioh Scrolls](/yugioh-scrolls)** takes place in a slightly different setting than the original anime and manga. Characters are usually the same or as close as possible to the original sources, but there might be some differences due to how the story plays out._


# Index

- [Characters from the 🗝️ Millennium Chapter](#characters-from-the-millennium-chapter)
- [Characters from the 👑 King's Chapter](#characters-from-the-kings-chapter)
- [Characters from the 📚 Academy Chapter](#characters-from-the-academy-chapter)
- [Characters from the 🔮 Signer's Chapter](#characters-from-the-signers-chapter)
- [Characters from the 🔢 Numbers Chapter](#characters-from-the-numbers-chapter)
- [Characters from the 🌌 Dimensional Chapter](#characters-from-the-dimensional-chapter)
- [Characters from the 🌐 Virtual Chapter](#characters-from-the-virtual-chapter)
- [Characters from Duel Anecdotes](#characters-from-duel-anecdotes)
- [Characters from Duel Visions](#characters-from-duel-visions)

---

## Characters from the Millennium Chapter

#### Protagonist

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/yugi.jpg" caption="Yugi" width="160" >}}
{{< imagetext src="/img/chars/theotheryugi.jpg" caption="The Other Yugi" width="160" >}}
</div>

##### Main & Recurring Cast

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/kaiba.jpg" caption="Kaiba" width="160" >}}
{{< imagetext src="/img/chars/jonouchi.jpg" caption="Jonouchi" width="160" >}}
{{< imagetext src="/img/chars/anzu.jpg" caption="Anzu" width="160" >}}
{{< imagetext src="/img/chars/honda.jpg" caption="Honda" width="160" >}}
{{< imagetext src="/img/chars/bakura.jpg" caption="Bakura" width="160" >}}
</div>

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/mai.jpg" caption="Mai" width="160" >}}
{{< imagetext src="/img/chars/mokuba.jpg" caption="Mokuba" width="160" >}}
{{< imagetext src="/img/chars/otogi.jpg" caption="Otogi" width="160" >}}
{{< imagetext src="/img/chars/shizuka.jpg" caption="Shizuka" width="160" >}}
{{< imagetext src="/img/chars/sugoroku.jpg" caption="Sugoroku" width="160" >}}
</div>

##### Duelist Kingdom Arc

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/pegasus.jpg" caption="Pegasus" width="160" >}}
{{< imagetext src="/img/chars/haga.jpg" caption="Haga" width="160" >}}
{{< imagetext src="/img/chars/ryuzaki.jpg" caption="Ryuzaki" width="160" >}}
{{< imagetext src="/img/chars/kajiki.jpg" caption="Kajiki" width="160" >}}
{{< imagetext src="/img/chars/impersonator.jpg" caption="Impersonator of Death" width="160" >}}
{{< imagetext src="/img/chars/panik.jpg" caption="Panik" width="160" >}}
{{< imagetext src="/img/chars/kotsuzuka.jpg" caption="Kotsuzuka" width="160" >}}
</div>

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/keith.jpg" caption="Keith" width="160" >}}
{{< imagetext src="/img/chars/tom.jpg" caption="Tom" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="Takaido" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="Satake" width="160" >}}
{{< imagetext src="/img/chars/brothermei.jpg" caption="Brother Mei" width="160" >}}
{{< imagetext src="/img/chars/brotherkyu.jpg" caption="Brother Kyu" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="Gravekeeper" width="160" >}}
</div>

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/cyndia.jpg" caption="Cyndia" width="160" >}}
{{< imagetext src="/img/chars/crocketts.jpg" caption="Mr. Crocketts" width="160" >}}
</div>

##### Museum Arc

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/shadi.jpg" caption="Shadi" width="160" >}}
{{< imagetext src="/img/chars/ishizu.jpg" caption="Ishizu" width="160" >}}
{{< imagetext src="/img/chars/rebecca.jpg" caption="Rebecca" width="160" >}}
{{< imagetext src="/img/chars/arthur.jpg" caption="Arthur" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="Professor Yoshimori" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="Curator Kanekura" width="160" >}}
{{< imagetext src="/img/chars/stepjohnny.jpg" caption="Step Johnny" width="160" >}}
</div>

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/noah.jpg" caption="Noah" width="160" >}}
{{< imagetext src="/img/chars/gozaburo.jpg" caption="Gozaburo" width="160" >}}
{{< imagetext src="/img/chars/oshita.jpg" caption="Oshita (The Big Five)" width="160" >}}
{{< imagetext src="/img/chars/otaki.jpg" caption="Otaki (The Big Five)" width="160" >}}
{{< imagetext src="/img/chars/ooka.jpg" caption="Ooka (The Big Five)" width="160" >}}
{{< imagetext src="/img/chars/ota.jpg" caption="Ota (The Big Five)" width="160" >}}
{{< imagetext src="/img/chars/daimon.jpg" caption="Daimon (The Big Five)" width="160" >}}
</div>

##### Battle City Arc

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/marik.jpg" caption="Marik" width="160" >}}
{{< imagetext src="/img/chars/rishid.jpg" caption="Rishid" width="160" >}}
{{< imagetext src="/img/chars/rarehunter.jpg" caption="Rare Hunter" width="160" >}}
{{< imagetext src="/img/chars/pandora.jpg" caption="Pandora" width="160" >}}
{{< imagetext src="/img/chars/thequietone.jpg" caption="The Quiet One" width="160" >}}
{{< imagetext src="/img/chars/maskofdarkness.jpg" caption="Mask of Darkness" width="160" >}}
{{< imagetext src="/img/chars/maskoflight.jpg" caption="Mask of Light" width="160" >}}
</div>

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/esperroba.jpg" caption="Esper Roba" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="Koji Nagumo" width="160" >}}
{{< imagetext src="/img/chars/magnum.jpg" caption="Jean-Claude Magnum" width="160" >}}
{{< imagetext src="/img/chars/isono.jpg" caption="Isono" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="Tournament Duelist" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="Tournament Duelist" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="Tournament Duelist" width="160" >}}
</div>

##### Wicked Arc

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/yako.jpg" caption="Yako Tenma" width="160" >}}
{{< imagetext src="/img/chars/gekko.jpg" caption="Gekko Tenma" width="160" >}}
{{< imagetext src="/img/chars/richiemerced.jpg" caption="Richie Merced" width="160" >}}
{{< imagetext src="/img/chars/scottdepre.jpg" caption="Scott Depre" width="160" >}}
{{< imagetext src="/img/chars/tillamook.jpg" caption="Tilla Mook" width="160" >}}
{{< imagetext src="/img/chars/tedbanias.jpg" caption="Ted Banias" width="160" >}}
{{< imagetext src="/img/chars/maicokato.jpg" caption="Maico Kato" width="160" >}}
</div>

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/petecoppermine.jpg" caption="Pete Coppermine" width="160" >}}
{{< imagetext src="/img/chars/willamette.jpg" caption="Willa Mette" width="160" >}}
{{< imagetext src="/img/chars/kirkdixon.jpg" caption="Kirk Dixon" width="160" >}}
{{< imagetext src="/img/chars/reikokitamori.jpg" caption="Reiko Kitamori" width="160" >}}
{{< imagetext src="/img/chars/rafael.jpg" caption="Rafael" width="160" >}}
{{< imagetext src="/img/chars/valon.jpg" caption="Valon" width="160" >}}
{{< imagetext src="/img/chars/amelda.jpg" caption="Amelda" width="160" >}}
</div>

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/dartz.jpg" caption="Dartz" width="160" >}}
{{< imagetext src="/img/chars/gurimo.jpg" caption="Gurimo" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="Orichalcos Police" width="160" >}}
</div>

## Characters from the King's Chapter

##### Protagonist

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/yugi2.jpg" caption="Yugi" width="160" >}}
</div>

## Characters from the Academy Chapter

##### Protagonist

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/yudai.jpg" caption="Yudai" width="160" >}}
</div>

## Characters from the Signer's Chapter

##### Protagonist

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/yusei.jpg" caption="Yusei" width="160" >}}
</div>

## Characters from the Numbers Chapter

##### Protagonist

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/yuma.jpg" caption="Yuma" width="160" >}}
</div>

## Characters from the Dimensional Chapter

##### Protagonist

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/yuzu.jpg" caption="Yuzu" width="160" >}}
</div>

## Characters from the Virtual Chapter

##### Protagonist

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/yusaku.jpg" caption="Yusaku" width="160" >}}
</div>

## Characters from Duel Anecdotes

##### Characters

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/0.jpg" caption="?" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="?" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="?" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="?" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="?" width="160" >}}
</div>

## Characters from Duel Visions

##### Characters

<div style="display: flex; justify-content: left; gap: 5px;">
{{< imagetext src="/img/chars/0.jpg" caption="?" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="?" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="?" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="?" width="160" >}}
{{< imagetext src="/img/chars/0.jpg" caption="?" width="160" >}}
</div>

---

<div style="display: flex; justify-content: center; gap: 5px;">
{{< imagetext src="/img/storyimg/M1.png" link="/yugioh-scrolls/" alt="Chapters" caption="Chapters" width="140" >}}
{{< imagetext src="/img/storyimg/M2.png" link="/scrolls/world/" alt="Setting" caption="Setting" width="140" >}}
{{< imagetext src="/img/storyimg/M3.png" link="/scrolls/timeline/" alt="Timeline" caption="Timeline" width="140" >}}
{{< imagetext src="/img/storyimg/M4.png" link="/scrolls/duels/" alt="Duels" caption="Duels" width="140" >}}
{{< imagetext src="/img/storyimg/M5.png" link="/scrolls/characters/" alt="Characters" caption="Characters" width="140" >}}
</div>

